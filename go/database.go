...
func AuthenticateUser(db *sql.DB, username, password string) (int, bool, error) {
	var userId int
	var hashedPassword string
	err := db.QueryRow("SELECT id, password FROM USER WHERE pseudo = ?", username).Scan(&userId, &hashedPassword)
	if err != nil {
		return 0, false, err
	}
